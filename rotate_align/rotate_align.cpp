...
bool RotateAlign::parserProgramOptions(int argc, char *argv[]) {
  try {
    ...
    ("capture_name,n", po::value<std::string>(&this->capture_name)->default_value(""), "Prefix of saved ply files")
...
void RotateAlign::transformPointCloud(size_t index) {
  ...
  pcl::transformPointCloudWithNormals(*point_clouds[index], *transformed_point_clouds[index], transform);
  // Rotate around center
  transform = Eigen::Affine3f::Identity();
  Eigen::Matrix3f rotation(Eigen::AngleAxisf(-DEG2RAD(index * capture_step + index * 1.17), Eigen::Vector3f::UnitY()));
  transform.rotate(rotation);
  pcl::transformPointCloudWithNormals(*transformed_point_clouds[index], *transformed_point_clouds[index], transform);
}
