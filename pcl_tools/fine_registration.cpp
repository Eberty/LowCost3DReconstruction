...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    ("max_iterations", po::value<int>(&max_iterations)->default_value(50), "The maximum number of iterations the internal optimization should run for")
    ("transformation_epsilon", po::value<double>(&transformation_epsilon)->default_value(1e-9), "Maximum allowable difference between two consecutive transformations to be considered as having converged")
    ("euclidean_fitness_epsilon", po::value<double>(&euclidean_fitness_epsilon)->default_value(0.001), "Maximum allowed Euclidean error between two consecutive steps in the ICP loop, before the algorithm is considered to have converged");
